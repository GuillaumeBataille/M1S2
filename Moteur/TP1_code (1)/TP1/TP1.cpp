...
// Declare global variables for storing the previous mouse position
...
void initPlane(std::vector<unsigned short> &indices, std::vector<std::vector<unsigned short>> &triangles, std::vector<glm::vec3> &indexed_vertices,std::vector<glm::vec2> &uv, int resolution, int size, bool randomheight)
{
    ...
    for (int i = 0; i < resolution + 1; i++)
    {
        for (int j = 0; j < resolution + 1; j++)
        {
            ...
            x = j* pas;
            y= randomheight? ((double) rand() / (RAND_MAX)): 0;
            z = i * pas;
            std::cout << "vecteur courant = (" << x << "," << y << z <<") et on le recupère dans index_vertices" << std::endl;
            indexed_vertices.push_back(glm::vec3(x - offset,0, z-offset));
            u = j * uv_pas;
            v = i* uv_pas;
            uv.push_back(glm::vec2(u - offset, v - offset));
        }
    }
...
int main(void)
{
    ...
    initPlane(indices, triangles, indexed_vertices, uv,RESOLUTION, SIZE, true); // Initiation du plan
...
{
    mat_t = glm::rotate(mat_t, glm::radians(1.0f), glm::vec3(1.0f, 0.0f, 0.0f));
}
...
{
    mat_t = glm::rotate(mat_t, glm::radians(1.0f), glm::vec3(0.0f, 1.0f, 0.0f));
}
...
if (isOrbiting)
{
    float cameraOrbitSpeed = 0.1f; // adjust this value to change the orbit speed
    ...
    float verticalAngle = atan2(camera_position.y - camera_target.y, radius);
